...
// Bilingual helper, consistent with src/i18n.js (localStorage "language").
const getLanguage = () =>
  localStorage.getItem("language") || "de";

const v = (de, en) => (getLanguage().startsWith("de") ? de : en);
...
export function downloadIcs(res) {
  ...
  const lines = [
    ...
    `DESCRIPTION:${esc(v(
      ...
      `${res.persons === 1 ? "1 Seat" : `${res.persons} seats`} at the Mischtisch · Seat ${res.seats.map((s) => s + 1).join(", ")}${res.aktion ? ` · ${res.aktion}` : ""}`,
    ))}`,
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n");
